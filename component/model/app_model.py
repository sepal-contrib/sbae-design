...
class AppModel(sumodel.Model):
    ...
    raw_area_df = traitlets.Any(None).tag(sync=True) # Holds result from compute_map_area
    final_area_df = traitlets.Any(None).tag(sync=True) # Holds area_df after class name edits
    sample_points_df = traitlets.Any(None).tag(sync=True) # Holds generated sample points DF
    alert_message = traitlets.Unicode("Select a map file and click 'Load File'.").tag(sync=True)
