...
try:
    ...
    print(
        f"CRITICAL WARNING (map_utils): FAILED to import rasterio or rasterio.warp. Error: {e}"
    )  # Make it stand out
...
try:
    ...
    print(
        "Warning (map_utils): Pillow not found. Raster display may rely on matplotlib."
    )
...
def _add_overlay_layer(
    ...
    try:
        ...
        if map_file_str.lower().endswith(raster_extensions):
            if not _rasterio_available:
                print(
                    "Error (map_utils._add_overlay_layer): Rasterio library needed for raster overlay."
                )
                return None
            if not _matplotlib_available:
                print(
                    "Error (map_utils._add_overlay_layer): Matplotlib library needed for raster coloring."
                )
                return None
            ...
            if (
                ...
                print(
                    f"DEBUG: (map_utils._add_overlay_layer) Processing Raster {basename} (ImageOverlay/DataURL)..."
                )
                src_raster = None
                data_raster = None
                image_url = None  # Renamed to avoid conflict
                try:
                    with rasterio.open(map_file_str) as src_raster:
                        ...
                        if h > max_dim or w > max_dim:
                            ...
                            print(
                                f"DEBUG: (map_utils._add_overlay_layer) Downsampling {h}x{w} raster to {out_h}x{out_w} for display."
                            )
                            ...
                            print(
                                "DEBUG: (map_utils._add_overlay_layer) Using full resolution for display."
                            )
                    ...
                    if legend_data is not None and "map_code" in legend_data.columns:
                        ...
                        common_codes = np.intersect1d(
                            valid_codes, raster_codes_present, assume_unique=True
                        )

                        if len(common_codes) > 0:
                            print(
                                f"DEBUG: (map_utils._add_overlay_layer) Colorizing raster based on {len(common_codes)} common codes found in legend."
                            )
                            color_map_hex = generate_class_color_map(common_codes)

                            def hex_to_rgba(hex_color, alpha=0.7):
                                try:
                                    rgb_color = matplotlib.colors.to_rgb(
                                        hex_color
                                    )  # Renamed variable
                                    ...
                                    print(
                                        f"Warning: Invalid hex color '{hex_color}'. Using transparent gray."
                                    )
                                    return (128, 128, 128, 0)

                            color_map_rgba = {
                                code: hex_to_rgba(hex_c)
                            ...
                            print(
                                "DEBUG: (map_utils._add_overlay_layer) Converting colored array to PNG bytes..."
                            )
                            buffer = BytesIO()
                            plt.imsave(
                                buffer, rgba_image, format="png"
                            )  # plt from import
                            buffer.seek(0)
                            png_bytes = buffer.read()
                            buffer.close()
                            print(
                                f"DEBUG: (map_utils._add_overlay_layer) Generated PNG ({len(png_bytes)} bytes)."
                            )

                            png_base64 = base64.b64encode(png_bytes).decode("utf-8")
                            image_url = f"data:image/png;base64,{png_base64}"
                            print(
                                f"DEBUG: (map_utils._add_overlay_layer) Created Data URL (approx len: {len(image_url)})."
                            )
                        else:
                            print(
                                "Warning (map_utils._add_overlay_layer): No common codes found between raster data and legend. Cannot colorize."
                            )
                    else:
                        print(
                            "Warning (map_utils._add_overlay_layer): Legend data or map_code column missing. Cannot colorize raster."
                        )

                    if image_url:  # Check if image_url was successfully created
                        print(
                            "DEBUG: (map_utils._add_overlay_layer) Adding ImageOverlay to map..."
                        )
                        ...
                        print(
                            "DEBUG: (map_utils._add_overlay_layer) Successfully ADDED ImageOverlay."
                        )
                        created_layer = img_overlay
                    else:
                        print(
                            "Warning (map_utils._add_overlay_layer): No image URL generated, skipping ImageOverlay."
                        )
                except MemoryError:
                    print(
                        "ERROR (map_utils._add_overlay_layer): Insufficient memory to process raster for display."
                    )
                    ...
                    if "data_raster" in locals() and data_raster is not None:
                        del data_raster
                    if "rgba_image" in locals() and rgba_image is not None:
                        del rgba_image
            else:
                print(
                    "Warning (map_utils._add_overlay_layer): Skipping Raster overlay - valid EPSG:4326 bounds were not provided."
                )
            ...
            if vector_gdf is not None and not vector_gdf.empty:
                print(
                    f"DEBUG: (map_utils._add_overlay_layer) Processing Vector Overlay for {basename}..."
                )
                try:
                    ...
                    if attr_cols:
                        map_code_col_vec = attr_cols[
                            0
                        ]  # Use local var to avoid conflict
                        print(
                            f"DEBUG: (map_utils._add_overlay_layer) Using vector column '{map_code_col_vec}' for styling."
                        )
                        if map_code_col_vec in vector_gdf.columns:
                            if (
                                ...
                                vector_codes_present = (
                                    vector_gdf[map_code_col_vec].dropna().unique()
                                )
                                common_codes_vec = np.intersect1d(
                                    valid_codes_vec,
                                    vector_codes_present,
                                    assume_unique=True,
                                )
                                ...
                                if len(common_codes_vec) > 0:
                                    print(
                                        f"DEBUG: (map_utils._add_overlay_layer) Colorizing vector based on {len(common_codes_vec)} common codes."
                                    )
                                    color_map_hex_vec = generate_class_color_map(
                                        common_codes_vec
                                    )
                                    color_map_lookup_vec = {
                                        code: hex_c
                                        for code, hex_c in color_map_hex_vec.items()
                                    }
                                    fallback_color_vec = "#808080"

                                    def vector_style_callback(
                                        feature,
                                        cmap=color_map_lookup_vec,
                                        code_col_cb=map_code_col_vec,
                                    ):  # Renamed var in CB
                                        code_val = feature["properties"].get(
                                            code_col_cb
                                        )  # Renamed var in CB
                                        color_fill = cmap.get(
                                            code_val, fallback_color_vec
                                        )  # Renamed var in CB
                                        return {
                                    ...
                                    print(
                                        "Warning (map_utils._add_overlay_layer): No common codes found for vector styling. Using default style."
                                    )
                            else:
                                print(
                                    "Warning (map_utils._add_overlay_layer): Legend data missing for vector coloring. Using default style."
                                )
                        else:
                            print(
                                f"Warning (map_utils._add_overlay_layer): Styling column '{map_code_col_vec}' not found in vector. Using default style."
                            )
                    else:
                        print(
                            "Warning (map_utils._add_overlay_layer): Vector has no attribute columns for styling. Using default style."
                        )

                    print(
                        "DEBUG: (map_utils._add_overlay_layer) Converting GDF to GeoJSON dict..."
                    )
                    vector_gdf_clean = vector_gdf[
                        vector_gdf.is_valid & ~vector_gdf.is_empty
                    ]
                    if vector_gdf_clean.empty:
                        print(
                            "Warning (map_utils._add_overlay_layer): No valid geometries remain after cleaning GDF."
                        )
                        return None
                    geojson_data = json.loads(vector_gdf_clean.to_json())
                    print(
                        "DEBUG: (map_utils._add_overlay_layer) Creating GeoJSON layer..."
                    )
                    ...
                    print(
                        f"DEBUG: (map_utils._add_overlay_layer) Successfully ADDED GeoJSON layer '{layer_name}'."
                    )
                ...
                print(
                    "Warning (map_utils._add_overlay_layer): Skipping Vector overlay - valid GeoDataFrame was not provided or was empty."
                )
        else:
            print(
                f"Warning (map_utils._add_overlay_layer): File type not recognized for overlay: {basename}"
            )
    ...
    if created_layer:
        print(
            f"DEBUG: (map_utils._add_overlay_layer) Overlay layer object '{getattr(created_layer, 'name', 'Unnamed')}' CREATED."
        )
    else:
        print("DEBUG: (map_utils._add_overlay_layer) Overlay layer object NOT created.")
