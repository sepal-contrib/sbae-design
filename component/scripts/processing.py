...
try:
    ...
    print(
        f"CRITICAL WARNING (processing): FAILED to import pyproj. Error: {e}"
    )  # Make it stand out
    ...
    if _scipy_available:
        try:
            ...
            return (
                ndtri(p)
                if 0 < p < 1
                else (_ for _ in ()).throw(ValueError(f"Invalid probability {p}"))
            )
        ...
        raise ValueError(
            f"Accurate Z-score for {confidence_level} requires scipy, or use 0.90, 0.95, 0.99."
        )
...
def get_output_dir(input_file_path):
    ...
    if (
        input_file_path
        and isinstance(input_file_path, (str, Path))
        and Path(input_file_path).is_file()
    ):
        try:
            ...
            print(
                f"Error creating output directory for {input_file_path}: {e}"
            )  # Clarified error
            return None
    else:
...
def compute_map_area(input_map_file_path):
    ...
    area_df = None
    if (
        not input_map_file_path
        or not isinstance(input_map_file_path, (str, Path))
        or not Path(input_map_file_path).exists()
    ):
    ...
    if file_path.suffix.lower() in raster_extensions:
        ...
        try:
            with rasterio.open(file_path) as src:
                ...
                if crs and crs.is_geographic:
                    print("Warning: CRS geographic. Area in sq. degrees.")
                elif not crs:
                ...
                if nodata is not None:
                    valid_mask &= (
                        ~np.isnan(unique_values)
                        if np.isnan(nodata)
                        else (unique_values != nodata)
                    )
                ...
                area_df = pd.DataFrame(
                    {"map_code": unique_values, "map_area": counts * pixel_area}
                )
        ...
        try:
            ...
            assert (
                geom_col in gdf.columns
                and not gdf[geom_col].isnull().all()
                and not gdf[geom_col].is_empty.all()
            ), "No valid geometries."
            ...
            assert (
                map_code_col in gdf.columns and not gdf[map_code_col].isnull().all()
            ), f"Map code column '{map_code_col}' invalid."
            ...
            try:
                ...
                gdf["map_area"] = (
                    gdf["map_area"].replace([np.inf, -np.inf], np.nan).fillna(0)
                )
                gdf = gdf[
                    (gdf["map_area"] > 0) & (gdf[map_code_col].notna())
...
def generate_sample_points(map_file, final_area_df_col_indexed, n_samples_per_class):
    ...
    if final_area_df_col_indexed is None or not all(
        c in final_area_df_col_indexed.columns for c in ["map_code", "map_edited_class"]
    ):
        print("Error: final_area_df for sample generation is invalid.")
        return pd.DataFrame()

    class_lookup = final_area_df_col_indexed.set_index("map_code")[
        "map_edited_class"
    ].to_dict()
    ...
    try:
        if map_file_path.suffix.lower() in raster_extensions:
            ...
            with rasterio.open(map_file_path) as src:
                ...
                if (
                    source_crs
                    and _pyproj_available
                    and CRS.from_user_input(source_crs).to_epsg() != target_crs_epsg
                ):
                    try:
                        transformer_proj = Transformer.from_crs(
                            source_crs, f"EPSG:{target_crs_epsg}", always_xy=True
                        )
                        print("Reprojection transformer created for raster.")
                    except Exception as e:
                        print(f"Warning: Failed to create transformer for raster: {e}.")
                elif not source_crs:
                    print("Warning: No CRS for raster. Assuming EPSG:4326.")
                ...
                for map_code, n_samples in n_samples_per_class.items():
                    ...
                    edited_class_str = (
                        str(edited_class)
                        if pd.notna(edited_class)
                        else f"Code {map_code}"
                    )
                    ...
                    if num_available_pixels == 0:
                        print(
                            f"Warning: No pixels for {map_code} ({edited_class_str})."
                        )
                        continue
                    ...
                    if num_samples_to_take < n_samples:
                        print(
                            f"Warning: Requested {n_samples} for {edited_class_str}, only {num_available_pixels} available."
                        )

                    sampled_indices = random.sample(
                        range(num_available_pixels), num_samples_to_take
                    )
                    ...
                    for r_idx, c_idx in zip(sampled_rows, sampled_cols):
                        x_coord, y_coord = transform_affine * (
                            c_idx + 0.5,
                            r_idx + 0.5,
                        )  # Center of pixel
                        ...
                        if transformer_proj:
                            try:
                                lon, lat = transformer_proj.transform(x_coord, y_coord)
                            except Exception as proj_error:
                                print(
                                    f"Warning: Reproject failed for raster point ({x_coord},{y_coord}): {proj_error}. Skipping."
                                )
                                continue
                        sample_points_list.append(
                            {
                                ...
                                "longitude": lon,
                                "map_code": map_code,
                                "map_edited_class": edited_class_str,
                            }
                        )
                    print(
                        f"Generated {num_samples_to_take} points for class '{edited_class_str}' (Raster)"
                    )
                del data
            ...
            if gdf.empty:
                raise ValueError("Input vector file empty for sample generation.")
            ...
            attribute_cols = [col for col in gdf.columns if col != geom_col]
            map_code_col = (
                attribute_cols[0] if attribute_cols else None
            )  # Assuming first attr is class
            assert (
                map_code_col and map_code_col in gdf.columns
            ), f"Map code column '{map_code_col}' not valid for vector."
            ...
            if (
                ...
                try:
                    ...
                    print("Reprojection transformer created for vector.")
                except Exception as e:
                    print(f"Warning: Failed to create transformer for vector: {e}.")
            elif not source_crs:
                print("Warning: No CRS for vector. Assuming EPSG:4326.")
            ...
            for map_code, n_samples in n_samples_per_class.items():
                ...
                edited_class = class_lookup.get(map_code, f"Code {map_code}")
                edited_class_str = (
                    str(edited_class) if pd.notna(edited_class) else f"Code {map_code}"
                )
                class_gdf = gdf[gdf[map_code_col] == map_code].copy()
                class_gdf[geom_col] = class_gdf[geom_col].buffer(0)
                class_gdf = class_gdf[
                    class_gdf[geom_col].is_valid & ~class_gdf[geom_col].is_empty
                ]

                if class_gdf.empty:
                    print(
                        f"Warning: No valid/non-empty geometries for {map_code} ({edited_class_str}) in vector."
                    )
                    continue
                ...
                if not (np.all(np.isfinite(total_bounds)) and len(total_bounds) == 4):
                    print(
                        f"Warning: Invalid bounds {total_bounds} for {edited_class_str} (vector). Skipping."
                    )
                    continue
                minx, miny, maxx, maxy = total_bounds
                if not (minx <= maxx and miny <= maxy):
                    print(
                        f"Warning: Illogical bounds [{minx},{miny},{maxx},{maxy}] for {edited_class_str} (vector). Skipping."
                    )
                    continue
                ...
                print(
                    f"Attempting {n_samples} points for class '{edited_class_str}' (Vector)..."
                )
                ...
                for _ in range(max_attempts):
                    ...
                    if class_gdf.contains(point).any():
                        ...
                        if transformer_proj:
                            try:
                                lon, lat = transformer_proj.transform(rand_x, rand_y)
                            except Exception as proj_error:
                                print(
                                    f"Warning: Reproject failed for vector point ({rand_x},{rand_y}): {proj_error}. Skipping."
                                )
                                continue
                        sample_points_list.append(
                            {
                ...
                print(
                    f"Generated {samples_generated}/{n_samples} points for class '{edited_class_str}' (Vector)."
                )
                if samples_generated < n_samples:
                    print(
                        f"Warning: Low generation rate for {edited_class_str} (Vector) after {max_attempts} attempts."
                    )
            del gdf, class_gdf
        else:
...
def output_sample_points(df_samples_to_output: pd.DataFrame, output_dir_path: str):
    ...
    if not output_dir_path or not os.path.isdir(output_dir_path):
        print_msg(
            f"Error: Output directory '{output_dir_path}' invalid for saving samples."
        )
        return
    ...
    try:
        ...
        df_samples_to_output["latitude"] = pd.to_numeric(
            df_samples_to_output["latitude"], errors="coerce"
        )
        df_samples_to_output["longitude"] = pd.to_numeric(
            df_samples_to_output["longitude"], errors="coerce"
        )
        valid_points_gdf = df_samples_to_output.dropna(subset=["latitude", "longitude"])

        if valid_points_gdf.empty:
            print_msg(
                "Error: No valid coordinates found for GeoJSON output after cleaning."
            )
            return
        ...
        gdf_samples_out = gpd.GeoDataFrame(
            ...
            geometry=gpd.points_from_xy(
                valid_points_gdf.longitude, valid_points_gdf.latitude
            ),
...
def run_sample_design(
    map_file_path_str: str, final_area_df_input: pd.DataFrame, output_dir_str: str
):
    ...
    def report_summary(summary_str, msg_type="info"):
        if output_widget_ref:
            if (
                hasattr(output_widget_ref, "reset")
                and hasattr(output_widget_ref, "add_msg")
                and hasattr(output_widget_ref, "type")
            ):  # sui.Alert
    ...
    try:
        ...
        assert (
            0 < allowable_err < 1
        ), "Allowable Error (Overall) must be between 0 and 1."
        assert (
            0.8 <= confidence < 1
        ), "Confidence Level must be between 0.8 and 1 (exclusive)."
        assert min_n_class > 0, "Min Samples per Class must be positive."

        target_class_code, n_target_class_required, target_class_display_name = (
            None,
            0,
            "N/A",
        )
        if objective == "Target Class Precision":
            ...
            target_class_display_name = next(
                (
                    opt["text"]
                    for opt in target_class_widget.items
                    if opt["value"] == target_class_code
                ),
                f"Class {target_class_code}",
            )
            target_class_error = target_class_allowable_error_widget.v_model
            assert (
                0 < target_class_error < 1
            ), "Target Class Allowable Error must be between 0 and 1."
            assert (
                expected_user_accuracy_widgets
                and target_class_code in expected_user_accuracy_widgets
            ), f"Expected UA widget missing for target class {target_class_code}."
            target_class_ua = expected_user_accuracy_widgets[target_class_code].v_model
            assert (
                0 < target_class_ua < 1
            ), f"Expected UA for '{target_class_display_name}' (Code {target_class_code}) must be between 0 and 1."

            numerator_tc = z**2 * target_class_ua * (1 - target_class_ua)
            denominator_tc = target_class_error**2
            n_target_class_required = (
        ...
        assert (
            final_area_df_input is not None and not final_area_df_input.empty
        ), "Final class area data is missing or empty."
        design_df = final_area_df_input.copy()
        required_cols = ["map_code", "map_area", "map_edited_class"]
        assert all(
            c in design_df.columns for c in required_cols
        ), f"Area data missing one or more required columns: {required_cols}."
        design_df["map_area"] = pd.to_numeric(
            design_df["map_area"], errors="coerce"
        ).fillna(0)
        design_df = design_df[design_df["map_area"] > 0].reset_index(drop=True)
        assert (
            not design_df.empty
        ), "No classes found with positive area after cleaning."
        assert (
            not design_df["map_code"].duplicated().any()
        ), "Duplicate map codes found in final area data."
        ...
        n_target_formula = (
            ceil(numerator / denominator) if numerator > 0 and denominator > 0 else 0
        )
        ...
        if current_alloc_method == "Proportional":
            ...
            if Wj.sum() > 1e-9:
                Wj = Wj / Wj.sum()
                nj_initial = n_target_min_adjusted * Wj
            else:
                current_alloc_method = "Equal"
                report_progress(
                    "WARN: Proportional allocation failed. Falling back to Equal."
                )

        if (
            current_alloc_method == "Neyman"
        ):  # Could be after fallback from Proportional
            assert (
                expected_user_accuracy_widgets
            ), "Neyman allocation requires Expected User Accuracy widgets."
            Sj_values = {}
            valid_ua_count = 0
            for code, widget in expected_user_accuracy_widgets.items():
                ua = widget.v_model
                if 0 < ua < 1:
                    Sj_values[code] = np.sqrt(ua * (1 - ua))
                    valid_ua_count += 1
                else:
                    Sj_values[code] = 0.0
                    report_progress(
                        f"Warning: Expected UA for class {code} is {ua}. Setting std dev to 0."
                    )
            Sj = pd.Series(Sj_values).reindex(design_df_indexed.index).fillna(0)
            assert (
                valid_ua_count > 0
            ), "Neyman requires at least one class with valid Expected UA (0 < UA < 1)."
            ...
            if Wj.sum() > 1e-9:
                ...
                Wj = pd.Series(0.0, index=design_df_indexed.index)
                current_alloc_method = "Equal"
                report_progress(
                    "WARN: Total map area zero for Neyman. Falling back to Equal."
                )
            ...
            if current_alloc_method == "Neyman":  # Check again if still Neyman
                ...
                if sum_WkSk <= 1e-9:
                    if Wj.sum() > 1e-9:
                        nj_initial = n_target_min_adjusted * Wj
                        current_alloc_method = "Proportional"
                        report_progress(
                            "WARN: Neyman Sum(Wk*Sk) zero. Falling back to Proportional."
                        )
                    else:
                        current_alloc_method = "Equal"
                        report_progress(
                            "WARN: Neyman Sum(Wk*Sk) zero and Wj zero. Falling back to Equal."
                        )
                else:
        ...
        if current_alloc_method == "Equal":  # Handles explicit Equal or fallbacks
            nj_initial.loc[:] = (
                n_target_min_adjusted / num_classes if num_classes > 0 else 0.0
            )
        ...
        if (
            ...
            and target_class_code in nj_final.index
        ):
        ...
        summary_df["Initial Allocation (Float)"] = (
            summary_df["map_code"].map(nj_initial).round(2)
        )
        summary_df["Final Sample Count"] = (
            summary_df["map_code"].map(nj_final).astype(int)
        )
        summary_df.fillna(
        ...
        summary_output_str += (
            f"Allocation Method: {alloc_method} (Applied as: {current_alloc_method})\n"
        )
        ...
        if generated_samples is not None and not generated_samples.empty:
            sample_points_df = (
                generated_samples  # Set global for AppController to pick up
            )
            report_progress(f"Generated {len(generated_samples)} sample points.")
        else:
            sample_points_df = None
            report_progress(
                "Warning: Sample point generation failed or produced no points."
            )
        ...
        report_progress(err_msg)
        report_summary(
            f"--- Unexpected Error ---\n{e}\n\nPlease report this issue. Check progress log above.",
            msg_type="error",
        )
