...
logger.info(
    "SEPAL-UI Application components and Backend Functions (AppController) loading..."
)
...
class AppController:
    def __init__(self, model: AppModel):
        ...
        self.sample_points_layer = (
            None  # To keep track of the sample points layer on the map
        )
        ...
        self.per_class_sample_size_display = (
            None  # Initialize attribute for per-class display
        )
        self.per_class_sample_size_display_card_text = (
            None  # Initialize attribute for card text of per-class display
        )
        ...
        if hasattr(self, "objective_widget"):  # Ensure widget exists before observing
            self.objective_widget.observe(
                self._handle_objective_change, names="v_model"
            )
        else:
            logger.warning(
                "objective_widget not created at observer setup time in AppController __init__."
            )
    ...
    def _create_stage3_widgets(self):
        ...
        self.objective_widget = sui.Select(
            label="Calculation Objective",
            dense=True,
            outlined=True,
            class_="mb-2",
            clearable=False,
            items=["Overall Accuracy", "Target Class Precision"],
            v_model="Overall Accuracy",
        )
        self.target_class_widget = sui.Select(
            label="Target Class (for Precision Objective)",
            dense=True,
            outlined=True,
            class_="mb-2",
            items=[],
            v_model=None,
            clearable=True,
            disabled=True,  # Populated later
        )
        self.target_class_allowable_error_widget = v.Slider(
            label="Target Class Allowable Error (Ej)",
            min=0.01,
            max=0.25,
            step=0.001,
            v_model=0.05,
        ...
        self.target_overall_accuracy_widget = v.Slider(
            label="Target Overall Accuracy (OA)",
            min=0.01,
            max=0.99,
            step=0.01,
            v_model=0.90,
        ...
        self.confidence_level_widget = v.Slider(
            ...
            min=0.80,
            max=0.999,
            step=0.001,  # Max < 1 for Z-score calc
            v_model=0.95,
            thumb_label="always",
            class_="mt-2 mb-2",
        )
        ...
        self.allocation_method_widget = sui.Select(
            ...
            class_="mb-2",
            clearable=False,
            items=["Proportional", "Neyman", "Equal"],
            v_model="Proportional",
        )
        ...
        self.per_class_sample_size_display_card_text = v.CardText(
            children=["N/A"], class_="pa-1"
        )  # Content holder
        self.per_class_sample_size_display = v.Card(
            ...
            children=[
                v.CardTitle(
                    children=["Final Samples per Class"], class_="subtitle-1 pa-1 grey"
                ),  # Styled title
                self.per_class_sample_size_display_card_text,
            ],
        )
        ...
        self.stage3_calculate_btn = sui.Btn(
            "Calculate Sample & Generate Design",
            icon="mdi-cogs",
            class_="ma-2",
            disabled=True,
        )
        self.update_map_btn = sui.Btn(  # To show/refresh sample points on map
            "Show/Update Samples on Map",
            icon="mdi-map-marker-check",
            class_="ma-2",
            disabled=True,
        )
        self.show_map_overlay_btn = sui.Btn(  # To show/refresh base map overlay
            "Show/Update Map Overlay",
            icon="mdi-layers-refresh",
            class_="ma-2",
            disabled=True,
        )
        self.stage3_output = sui.Alert(
            class_="mt-2 pa-2", type="info"
        ).hide()  # For calculation feedback
    ...
    def _create_stepper(self):
        ...
        self.stepper_step_1 = v.StepperStep(
            step=1, complete=False, children=[self.step_1_title_text]
        )
        self.stepper_step_2 = v.StepperStep(
            step=2, complete=False, children=[self.step_2_title_text]
        )
        self.stepper_step_3 = v.StepperStep(
            step=3, complete=False, children=[self.step_3_title_text]
        )
        ...
        try:
            ...
            logger.critical(
                f"FATAL ERROR creating Step 1 UI content: {e_step1}", exc_info=True
            )
            step1_content_ui = [
                v.Html(
                    tag="p",
                    class_="error--text",
                    children=[f"Error creating UI for Step 1: {e_step1}. Check logs."],
                )
            ]

        self.stepper_content_1 = v.StepperContent(step=1, children=step1_content_ui)
        self.stepper_content_2 = v.StepperContent(
            step=2, children=[v.ProgressCircular(indeterminate=True, class_="ma-4")]
        )
        self.stepper_content_3 = v.StepperContent(
            step=3, children=[v.ProgressCircular(indeterminate=True, class_="ma-4")]
        )

        self.stepper = v.Stepper(
            class_="mb-5",
            v_model=self.model.current_step,  # Links to model's current_step
            children=[
                v.StepperHeader(
                    children=[
                        self.stepper_step_1,
                ...
                v.StepperItems(
                    children=[
                        self.stepper_content_1,
                        self.stepper_content_2,
                        self.stepper_content_3,
                    ]
                ),
            ],
        )
    ...
    def _create_step_1_content(self):
        ...
        self.load_btn = sui.Btn(
            "Load File, Compute Area & Display Map", icon="mdi-upload", class_="ma-2"
        )
        ...
        content_layout_step1 = v.Layout(
            ...
            children=[
                self.file_input,
                v.Layout(
                    row=True, class_="d-flex justify-center", children=[self.load_btn]
                ),
                self.alert_step1,
                self.map_widget,
            ],
    ...
    def _create_step_2_content(self):
        ...
        if self.model.raw_area_df is None or self.model.raw_area_df.empty:
            logger.error(
                "Cannot create Stage 2 content: No raw area data (raw_area_df) loaded from Step 1."
            )
            return [
                v.Html(
                    tag="p",
                    class_="warning--text",
                    children=[
                        "Error: No area data loaded from Step 1. Please go back and load a file."
                    ],
                )
            ]

        area_df_sorted_stage2 = (
            self.model.raw_area_df.copy().sort_values("map_code").reset_index(drop=True)
        )
            ...
            v.ListItemGroup(
                children=[
                    v.Html(
                        class_="pl-4 pt-2",
                        tag="b",
                        children=["Edit Class Names (Optional):"],
                    )
                ]
            )
        ]
            ...
            v.ListItemGroup(
                children=[
                    v.Html(
                        class_="pl-4 pt-2",
                        tag="b",
                        children=["Expected User's Accuracy (for Neyman Allocation):"],
                    )
                ]
            )
        ]
        ...
        for _, row_data in area_df_sorted_stage2.iterrows():
            ...
            name_tf = v.TextField(
                label=f"Name for Code {map_code_val}",
                v_model=initial_display_name,
                outlined=True,
                dense=True,
                class_="ma-2",
            )
            ...
            def _create_ua_label_observer(slider_widget, text_field_widget, code_value):
                def _observer_func(change):
                    new_name_val = (
                        text_field_widget.v_model.strip()
                        if text_field_widget.v_model
                        else f"Code {code_value}"
                    )
            ...
            name_tf.observe(
                _create_ua_label_observer(ua_slider, name_tf, map_code_val),
                names="v_model",
            )

        edit_names_card = v.Card(
            outlined=True,
            class_="mb-4",
            children=[v.List(dense=True, children=name_editor_items)],
        )
        edit_ua_card = v.Card(
            outlined=True,
            class_="mb-4",
            children=[v.List(dense=True, children=ua_slider_items)],
        )
        ...
        try:
            self.stage2_submit_btn.on_event("click", None, remove=True)
        except Exception as e_remove:
            logger.debug(
                f"Error removing previous Stage 2 submit handler (non-critical): {e_remove}"
            )
    ...
    def _create_step_3_content(self):
        logger.debug(
            "Creating Step 3 UI Content (Parameters, Calculation, Results Map)."
        )
        if self.model.final_area_df is None or self.model.final_area_df.empty:
            logger.error(
                "Cannot create Stage 3 content: Finalized class data (final_area_df) is not available from Step 2."
            )
            return [
                v.Html(
                    tag="p",
                    class_="warning--text",
                    children=[
                        "Error: Finalized class data not available. Please complete Step 2."
                    ],
                )
            ]
        ...
        class_summary_sheet = v.Sheet(
            ...
            children=[
                v.Html(
                    tag="b", children=["Final Class Summary (Map Code, Name, Area):"]
                ),
                v.Html(
                    tag="pre",
                    style_="color: black; font-family: monospace;",
                    children=[
                        self.model.final_area_df[
        ...
        params_column_layout = v.Layout(
            ...
            children=[
                ...
                self.target_class_widget,
                self.target_class_allowable_error_widget,
                v.Divider(class_="my-2"),
                v.Html(tag="i", children=["Standard Sampling Parameters:"]),
                self.target_overall_accuracy_widget,
                self.allowable_error_widget,
                self.confidence_level_widget,
                self.min_samples_per_class_widget,
        ...
        self.show_map_overlay_btn.disabled = not (
            self.model.file_path and self.base_map_overlay_layer is not None
        )

        try:
            self.stage3_calculate_btn.on_event("click", None, remove=True)
        except Exception as e_remove_calc:
            logger.debug(f"Error removing calc handler: {e_remove_calc}")
        self.stage3_calculate_btn.on_event("click", self._on_stage3_calculate)

        try:
            self.update_map_btn.on_event("click", None, remove=True)
        except Exception as e_remove_map:
            logger.debug(f"Error removing map update handler: {e_remove_map}")
        self.update_map_btn.on_event("click", self._on_update_map_click)

        try:
            self.show_map_overlay_btn.on_event("click", None, remove=True)
        except Exception as e_remove_overlay:
            logger.debug(f"Error removing show map overlay handler: {e_remove_overlay}")
        self.show_map_overlay_btn.on_event("click", self._on_show_map_overlay_click)

        if (
            not hasattr(self, "calculated_total_sample_size_display")
            or self.calculated_total_sample_size_display is None
        ):
            self._create_stage3_widgets()
        if (
            not hasattr(self, "per_class_sample_size_display")
            or self.per_class_sample_size_display is None
        ):
        ...
        content_layout_stage3 = v.Layout(
            ...
            children=[
                v.Html(
                    tag="h3",
                    children=["Stage 3: Set Sample Design Parameters & Calculate"],
                ),
                class_summary_sheet,
                v.Card(
                    outlined=True, class_="pa-4 mb-4", children=[params_column_layout]
                ),
                ...
                v.Layout(
                    ...
                    children=[
                        self.stage3_calculate_btn,
                        self.update_map_btn,
                        self.show_map_overlay_btn,
                    ],
                ),
                self.stage3_output,
        ...
        if hasattr(self, "stepper") and self.stepper:
            ...
            if new_step_val == 3 and hasattr(self, "map_widget") and self.map_widget:
                try:
                    ...
                    logger.error(
                        f"Error invalidating map size on step 3 activation: {e}"
                    )
        else:
            logger.warning(
                "Stepper widget not available when trying to update its value."
            )
    ...
    def _handle_objective_change(self, change):
        ...
        can_enable_target_specifics = (
            is_target_class_active
            and hasattr(self.target_class_widget, "items")
            and len(self.target_class_widget.items) > 0
            and self.target_class_widget.items[0].get("value") is not None
        )
        ...
        logger.debug(
            f"Objective changed to '{new_objective_val}'. Target class specific widgets enabled: {not self.target_class_widget.disabled}, Allowable error enabled: {not self.target_class_allowable_error_widget.disabled}"
        )
    ...
    def _update_target_class_options(self):
        ...
        if (
            self.model.final_area_df is not None
            and isinstance(self.model.final_area_df, pd.DataFrame)
            and not self.model.final_area_df.empty
        ):
            try:
                sorted_final_df = (
                ...
                class_choices = [
                    {
                        "text": f"{row['map_edited_class']} (Code: {row['map_code']})",
                        "value": row["map_code"],
                    }
                ...
                if not class_choices:
                    class_choices = [
                        {"text": "No classes eligible for targeting", "value": None}
                    ]

                target_select_widget.items = class_choices
                target_select_widget.v_model = (
                    None
                    if not class_choices or class_choices[0].get("value") is None
                    else class_choices[0]["value"]
                )
                logger.debug(
                    f"Updated target class items (count: {len(class_choices)}). Default selection: {target_select_widget.v_model}"
                )
            except Exception:
                logger.exception("Error formatting target class choices for dropdown:")
                target_select_widget.items = [
                    {"text": "Error loading classes", "value": None}
                ]
                target_select_widget.v_model = None
        else:
            logger.warning(
                "final_area_df is not available or empty. Setting target class options to 'No classes available'."
            )
            target_select_widget.items = [
                {"text": "No classes available", "value": None}
            ]
            target_select_widget.v_model = None
    ...
    def _on_load_click(self, widget, event, data):
        ...
        selected_filename_str = (
            os.path.basename(str(self.model.selected_file))
            if self.model.selected_file
            else "file"
        )
        self.model.alert_message = (
            f"Loading {selected_filename_str} and computing areas..."
        )
        ...
        if hasattr(self, "stepper_step_1"):
            self.stepper_step_1.complete = False
        if hasattr(self, "stepper_step_2"):
            self.stepper_step_2.complete = False
        if hasattr(self, "stepper_step_3"):
            self.stepper_step_3.complete = False

        layers_to_remove_on_load = []
        if self.sample_points_layer:
            layers_to_remove_on_load.append(self.sample_points_layer)
        if self.base_map_overlay_layer:
            layers_to_remove_on_load.append(self.base_map_overlay_layer)
        ...
        for layer_obj in layers_to_remove_on_load:
            try:
                if layer_obj in self.map_widget.layers:
                    ...
                    logger.debug(
                        f"Removed tracked map layer: {getattr(layer_obj, 'name', 'Unnamed Layer')}"
                    )
            except Exception as e_remove:
                logger.warning(
                    f"Could not remove tracked layer '{getattr(layer_obj, 'name', 'Unnamed Layer')}': {e_remove}"
                )
        ...
        for lyr in current_map_layers:
            if (
                isinstance(lyr, (ImageOverlay, GeoJSON, LayerGroup))
                and getattr(lyr, "base", False) is False
            ):
                if lyr not in layers_to_remove_on_load:
                    try:
                        self.map_widget.remove_layer(lyr)
                        logger.debug(
                            f"Removed other non-basemap layer: {getattr(lyr, 'name', 'Unknown Overlay')}"
                        )
                    except Exception as e_remove_other:
                        logger.warning(
                            f"Could not remove other layer '{getattr(lyr, 'name', 'Unknown Overlay')}': {e_remove_other}"
                        )

        if hasattr(self, "update_map_btn"):
            self.update_map_btn.disabled = True
        if hasattr(self, "show_map_overlay_btn"):
        ...
        try:
            ...
            logger.info(
                f"Selected file: {selected_file_path}. Type: {'Raster' if is_raster_file else 'Vector' if is_vector_file else 'Unknown'}"
            )
            logger.debug("About to load map file and compute areas...")

            area_data_computed = compute_map_area(selected_file_path)
            if not (
                area_data_computed is not None
                and isinstance(area_data_computed, pd.DataFrame)
                and not area_data_computed.empty
            ):
                raise ValueError(
                    "Area computation failed or returned empty. Cannot proceed."
                )

            if "map_edited_class" not in area_data_computed.columns:
                area_data_computed["map_edited_class"] = area_data_computed[
                    "map_code"
                ].astype(str)
            ...
            if is_raster_file:
                    ...
                    with rasterio.open(selected_file_path) as src_raster_ds:
                        if src_raster_ds.crs:
                            dst_bounds_transformed = rasterio.warp.transform_bounds(
                                src_raster_ds.crs,
                                f"EPSG:{map_utils_target_crs_epsg}",
                                *src_raster_ds.bounds,
                            )
                            ...
                            logger.debug(
                                f"Calculated EPSG:4326 bounds for raster overlay: {calculated_bounds_for_raster}"
                            )
                        else:
                            logger.warning(
                                "Raster missing CRS. Cannot calculate accurate bounds for map overlay."
                            )
                except Exception as bounds_calc_err:
                    logger.error(
                        f"Error calculating raster bounds for map overlay: {bounds_calc_err}",
                        exc_info=True,
                    )
                ...
                if calculated_bounds_for_raster:
                    new_overlay = _add_overlay_layer(
                        self.map_widget,
                        selected_file_path,
                        self.model.raw_area_df,
                        calculated_bounds_for_raster,
                        None,
                    )
                else:
                    logger.warning(
                        "Skipping raster overlay on map due to missing bounds or dependencies."
                    )
                ...
                try:
                    ...
                    if not gdf_temp.empty:
                        if gdf_temp.crs is None:
                            logger.warning(
                                f"Vector file '{basename_load}' has no CRS defined. Assuming EPSG:{map_utils_target_crs_epsg} for display, but this might be incorrect."
                            )
                        loaded_vector_gdf = gdf_temp.to_crs(
                            epsg=map_utils_target_crs_epsg
                        )
                        logger.debug(
                            f"Loaded and reprojected vector '{basename_load}' to EPSG:{map_utils_target_crs_epsg} for overlay."
                        )
                    else:
                        logger.warning(f"Vector file '{basename_load}' is empty.")
                except Exception as gdf_load_err:
                    logger.error(
                        f"Error loading/reprojecting vector '{basename_load}': {gdf_load_err}",
                        exc_info=True,
                    )
                ...
                if loaded_vector_gdf is not None:
                    ...
                    logger.warning(
                        "Skipping vector overlay on map due to loading error, empty GDF, or missing GeoPandas."
                    )
            ...
            if new_overlay:
                ...
                logger.info(
                    f"Map overlay '{getattr(new_overlay, 'name', 'Unnamed Overlay')}' added to map and tracked."
                )
                if hasattr(new_overlay, "bounds"):
                    ...
                    try:
                        ...
                        logger.debug(
                            f"Map zoomed to vector overlay bounds: {map_bounds_vec.tolist()}"
                        )
                    except Exception as fit_err:
                        logger.warning(
                            f"Could not automatically fit map to vector overlay bounds: {fit_err}"
                        )
            ...
            if hasattr(self, "stepper_step_1"):
                self.stepper_step_1.complete = True
            ...
            logger.error(
                f"Error during file loading or initial processing: {val_err}",
                exc_info=True,
            )
            ...
            logger.critical(
                f"Import error during file load, a critical library might be missing: {imp_err}",
                exc_info=True,
            )
            self.model.alert_type = "error"
            self.model.alert_message = (
                f"Critical library missing: {imp_err}. Cannot proceed."
            )
            if hasattr(self, "stepper_step_1"):
                self.stepper_step_1.complete = False
        except Exception as e_unexp:
            logger.exception(
                "An unexpected error occurred during file load and map display:"
            )
            self.model.alert_type = "error"
            self.model.alert_message = (
                f"An unexpected error occurred: {e_unexp}. Check logs for details."
            )
            if hasattr(self, "stepper_step_1"):
                self.stepper_step_1.complete = False
        finally:
    ...
    def _on_stage2_submit(self, widget, event, data):
        logger.info(
            "Stage 2 Submit button clicked. Processing class name edits and UA inputs."
        )
        widget.loading = True
        ...
        try:
            if not hasattr(self, "class_name_widgets") or not hasattr(
                self, "expected_ua_sliders"
            ):
                raise AttributeError(
                    "Stage 2 UI widgets (class_name_widgets or expected_ua_sliders) not properly initialized."
                )
            if self.model.raw_area_df is None or self.model.raw_area_df.empty:
                raise ValueError(
                    "Raw area data (raw_area_df from Step 1) is missing or empty. Cannot proceed."
                )

            edited_class_names_map = {
                code: (
                    name_widget.v_model.strip()
                    if name_widget.v_model and name_widget.v_model.strip()
                    else str(code)
                )
            ...
            if not self.expected_ua_sliders:
                logger.warning(
                    "Expected UA sliders dictionary is empty, though this might be fine if not using Neyman."
                )

            processed_df_stage2 = self.model.raw_area_df.copy()
            processed_df_stage2["map_edited_class"] = processed_df_stage2[
                "map_code"
            ].map(edited_class_names_map)

            if not (
                "map_edited_class" in processed_df_stage2.columns
                and not processed_df_stage2["map_edited_class"].isnull().any()
            ):
                raise AssertionError(
                    "Mapping edited class names to DataFrame failed or resulted in null values."
                )
            ...
            self.stage2_alert.children = [
                "Class data and user accuracies submitted. Proceed to Stage 3."
            ]
            if hasattr(self, "stepper_step_2"):
                self.stepper_step_2.complete = True
            if hasattr(self, "stepper_step_3"):
                self.stepper_step_3.complete = False
            ...
            self.stage2_alert.children = [
                f"Error submitting class data: {stage2_err}. Check logs."
            ]
            if hasattr(self, "stepper_step_2"):
                self.stepper_step_2.complete = False
            ...
            self.stage2_alert.type = "error"
            self.stage2_alert.children = [
                f"An unexpected error occurred: {e_unexp_s2}. Check logs."
            ]
            if hasattr(self, "stepper_step_2"):
                self.stepper_step_2.complete = False
        finally:
    ...
    def _on_stage3_calculate(self, widget, event, data):
        logger.info(
            "Stage 3 Calculate button clicked. Initiating sample design process."
        )
        widget.loading = True
        if hasattr(self, "update_map_btn"):
            self.update_map_btn.disabled = True
        ...
        self.stage3_output.children = [
            "Running sample design calculation... This may take a moment."
        ]

        if self.calculated_total_sample_size_display:
            self.calculated_total_sample_size_display.children = [
                "Calculated Total Sample Size: Calculating..."
            ]
        ...
        if self.per_class_sample_size_display:  # Reset per-class display
            if self.per_class_sample_size_display_card_text:
                self.per_class_sample_size_display_card_text.children = [
                    "Calculating..."
                ]
        ...
        if self.sample_points_layer is not None:
            try:
                ...
                logger.warning(
                    f"Could not remove old sample points layer: {e_remove_spl}"
                )
        ...
        try:
            ...
            proc.target_class_allowable_error_widget = (
                self.target_class_allowable_error_widget
            )
            ...
            if not self.model.file_path:
                raise ValueError("Input file path is missing in the model.")
            if self.model.final_area_df is None:
                raise ValueError("Final area data is missing in the model.")

            output_directory_path = get_output_dir(self.model.file_path)
            if not output_directory_path:
                raise RuntimeError(
                    "Failed to determine or create output directory for results."
                )
            ...
            if results and results.get("success"):
                ...
                if (
                    ...
                    and self.calculated_total_sample_size_display
                ):
                    self.calculated_total_sample_size_display.children = [
                ...
                if returned_samples_df is not None and isinstance(
                    returned_samples_df, pd.DataFrame
                ):
                    ...
                    if not self.model.sample_points_df.empty:
                        if "map_edited_class" in self.model.sample_points_df.columns:
                            try:
                                per_class_counts_df = (
                                    self.model.sample_points_df.groupby(
                                        "map_edited_class"
                                    )
                                    .size()
                                ...
                                if per_class_counts_df.empty:
                                    display_elements.append(
                                        v.Html(
                                            tag="p",
                                            children=[
                                                "No samples generated or classes found."
                                            ],
                                        )
                                    )
                                else:
                                    ...
                                    for _, row in per_class_counts_df.iterrows():
                                        ul_children.append(
                                            v.Html(
                                                tag="li",
                                                children=[
                                                    f"{html.escape(str(row['map_edited_class']))}: {row['n_samples']}"
                                                ],
                                            )
                                        )
                                    display_elements.append(
                                        v.Html(
                                            tag="ul",
                                            style_="list-style-type: disc; padding-left: 20px;",
                                ...
                                if (
                                    self.per_class_sample_size_display
                                    and self.per_class_sample_size_display_card_text
                                ):
                                    self.per_class_sample_size_display_card_text.children = (
                                        display_elements
                                    )
                                ...
                                logger.error(
                                    f"Error calculating/displaying per-class samples: {e_per_class}",
                                    exc_info=True,
                                )
                                if (
                                    self.per_class_sample_size_display
                                    ...
                                    self.per_class_sample_size_display_card_text.children = [
                                        "Error calculating per-class samples."
                                    ]
                                    self.per_class_sample_size_display.show()
                        else:  # 'map_edited_class' not in columns
                            logger.warning(
                                "'map_edited_class' not in sample_points_df, cannot show per-class counts."
                            )
                            if (
                                self.per_class_sample_size_display
                                and self.per_class_sample_size_display_card_text
                                ...
                                self.per_class_sample_size_display_card_text.children = [
                                    "Per-class data unavailable (missing class column)."
                                ]
                                self.per_class_sample_size_display.show()
                        ...
                        if (
                            ...
                            self.per_class_sample_size_display_card_text.children = [
                                "No samples generated to show per class."
                            ]
                            self.per_class_sample_size_display.show()
                    ...
                    if not self.model.sample_points_df.empty:
                        if hasattr(self, "stepper_step_3"):
                            self.stepper_step_3.complete = True
                        if hasattr(self, "update_map_btn"):
                        ...
                        if hasattr(self, "stepper_step_3"):
                            self.stepper_step_3.complete = True
                        logger.warning(
                            "Sample calculation successful, but no sample points were generated (final check)."
                        )
                else:
                    self.model.sample_points_df = None
                    if hasattr(self, "stepper_step_3"):
                        self.stepper_step_3.complete = False
                    logger.warning(
                        "Sample calculation successful, but no sample points DataFrame found in results."
                    )

                if (
                    self.model.sample_points_df is None
                    or self.model.sample_points_df.empty
                ):
                    if (
                        self.per_class_sample_size_display
                        and self.per_class_sample_size_display_card_text
                        ...
                        self.per_class_sample_size_display_card_text.children = [
                            "Samples per class: N/A (no samples data)."
                        ]
                        self.per_class_sample_size_display.show()

            else:
                error_msg_from_proc = (
                    results.get("error_message", "Calculation failed, check logs.")
                    if results
                    else "Calculation failed, check logs."
                )
                if self.calculated_total_sample_size_display:
                    self.calculated_total_sample_size_display.children = [
                ...
                if (
                    ...
                    self.per_class_sample_size_display_card_text.children = [
                        "Samples per class: Error in calculation."
                    ]
                    self.per_class_sample_size_display.show()
                logger.error(
                    f"Sample design calculation reported failure. Message: {error_msg_from_proc}"
                )
                if hasattr(self, "stepper_step_3"):
                    self.stepper_step_3.complete = False
                if hasattr(self, "update_map_btn"):
            ...
            logger.exception(
                "Error occurred in AppController during Stage 3 Calculate orchestration:"
            )
            self.stage3_output.type = "error"
            self.stage3_output.add_msg(
                f"Controller-level error during calculation: {e_calc_ctrl}. Check logs."
            )
            if self.calculated_total_sample_size_display:
                self.calculated_total_sample_size_display.children = [
                    "Calculated Total Sample Size: Error"
            ...
            if (
                ...
                self.per_class_sample_size_display_card_text.children = [
                    "Samples per class: Controller error."
                ]
                self.per_class_sample_size_display.show()
            if hasattr(self, "stepper_step_3"):
                self.stepper_step_3.complete = False
    ...
    def _on_show_map_overlay_click(self, widget, event, data):
        logger.info(
            "Show/Update Map Overlay button clicked. Refreshing base map overlay."
        )
        widget.loading = True
        self.stage3_output.reset().show()
        self.stage3_output.type = "info"
        ...
        if not self.model.file_path or not Path(self.model.file_path).exists():
            self.stage3_output.type = "error"
            self.stage3_output.add_msg(
                "No valid file loaded. Cannot display or refresh overlay."
            )
            logger.error(
                "Attempted to refresh map overlay with no valid file_path in model."
            )
            widget.loading = False
            return
        ...
        try:
            if self.base_map_overlay_layer is not None:
                try:
                    if self.base_map_overlay_layer in self.map_widget.layers:
                        ...
                        logger.debug(
                            "Removed existing base map overlay layer for refresh."
                        )
                    self.base_map_overlay_layer = None
                except Exception as e_remove_base:
                    logger.warning(
                        f"Could not remove existing base map overlay for refresh: {e_remove_base}"
                    )
            ...
            if is_raster_refresh:
                ...
                try:
                    with rasterio.open(self.model.file_path) as src_r:
                        if src_r.crs:
                            dst_b = rasterio.warp.transform_bounds(
                                src_r.crs,
                                f"EPSG:{map_utils_target_crs_epsg}",
                                *src_r.bounds,
                            )
                            raster_bounds_refresh = [
                                [dst_b[1], dst_b[0]],
                                [dst_b[3], dst_b[2]],
                            ]
                        else:
                            self.stage3_output.add_msg(
                                "Warning: Raster for refresh is missing CRS. Bounds may be inaccurate.",
                                "warning",
                            )
                            logger.warning("Raster for refresh missing CRS.")
                except Exception as bounds_err_r:
                    self.stage3_output.add_msg(
                        f"Error calculating raster bounds for refresh: {bounds_err_r}",
                        "error",
                    )
                    logger.error(
                        f"Error calculating raster bounds for refresh: {bounds_err_r}",
                ...
                try:
                    ...
                    if not gdf_loaded_r.empty:
                        if gdf_loaded_r.crs is None:
                            self.stage3_output.add_msg(
                                f"Warning: Vector '{basename_refresh}' for refresh has no CRS.",
                                "warning",
                            )
                            logger.warning(
                                f"Vector '{basename_refresh}' for refresh has no CRS."
                            )
                        vector_gdf_r = gdf_loaded_r.to_crs(
                            epsg=map_utils_target_crs_epsg
                        )
                except Exception as gdf_err_r:
                    self.stage3_output.add_msg(
                        f"Error loading/reprojecting vector for refresh: {gdf_err_r}",
                        "error",
                    )
                    logger.error(
                        f"Error loading/reprojecting vector for refresh: {gdf_err_r}",
                ...
                self.stage3_output.add_msg(
                    f"File type of '{basename_refresh}' not recognized for overlay refresh."
                )
                logger.warning(
                    f"Unrecognized file type for overlay refresh: {basename_refresh}"
                )
            ...
            if refreshed_overlay:
                ...
                self.stage3_output.add_msg(
                    f"Map overlay '{basename_refresh}' refreshed successfully."
                )
                logger.info(
                    f"Map overlay '{basename_refresh}' refreshed and added to map."
                )
            else:
                if self.stage3_output.type != "error":
                    self.stage3_output.type = "warning"
                self.stage3_output.add_msg(
                    f"Failed to refresh map overlay for '{basename_refresh}'. Check logs if not already detailed."
                )
                logger.warning(
                    f"Failed to refresh map overlay for '{basename_refresh}'."
                )

        except Exception as e_refresh:
            logger.exception(
                "An unexpected error occurred while refreshing map overlay:"
            )
            self.stage3_output.type = "error"
            self.stage3_output.add_msg(
                f"Unexpected error refreshing overlay: {e_refresh}. Check logs."
            )
        finally:
            widget.loading = False
    ...
    def _on_update_map_click(self, widget, event, data):
        ...
        try:
            ...
            self.stage3_output.type = "error"
            self.stage3_output.add_msg(
                f"Error updating sample display on map: {e_map_update}. Check logs."
            )
        finally:
            widget.loading = False
    ...
    def _update_map_display_ipyleaflet(self):
        ...
        if self.sample_points_layer is not None:
            try:
                ...
                logger.warning(
                    f"Could not remove previous sample points layer: {e_remove_samples}"
                )
        ...
        if not all(col in points_df_for_map.columns for col in req_cols_for_map):
            missing_cols_str = [
                col for col in req_cols_for_map if col not in points_df_for_map.columns
            ]
        ...
        try:
            ...
            logger.error(
                f"Error generating class colors for map sample points: {color_map_err}",
                exc_info=True,
            )
            class_colors_pts = {}
            default_marker_color = "#FF0000"
        ...
        for _, row_pt_data in points_df_for_map.iterrows():
            try:
                ...
                if not (
                    np.isfinite(lat_pt)
                    and np.isfinite(lon_pt)
                    and -90 <= lat_pt <= 90
                    and -180 <= lon_pt <= 180
                ):
                    logger.warning(
                        f"Skipping sample point with invalid/out-of-bounds coords: Lat={lat_pt}, Lon={lon_pt} for class {row_pt_data.get('map_edited_class', 'N/A')}"
                    )
                    continue
                ...
                logger.error(
                    f"Error creating map marker for sample point: {row_pt_data.to_dict()}. Error: {marker_create_err}",
                    exc_info=True,
                )

        if map_markers:
            self.sample_points_layer = LayerGroup(
                layers=map_markers, name="Sample Points"
            )
            self.map_widget.add_layer(self.sample_points_layer)
            logger.info(
                f"Added {valid_points_added_count} sample points to the map as a new layer."
            )
            ...
            try:
                ...
                logger.error(
                    f"Error calling invalidate_size on map widget: {e_invalidate}"
                )

            if (
                valid_points_added_count > 0
                and min_lat_pts <= max_lat_pts
                and min_lon_pts <= max_lon_pts
            ):
                bounds_to_fit_samples = [
                    [min_lat_pts, min_lon_pts],
                    [max_lat_pts, max_lon_pts],
                ]
                ...
                try:
                    ...
                    logger.debug(
                        f"Map view fitted to sample points bounds: {bounds_to_fit_samples}"
                    )
                except Exception as fit_bounds_err:
                    logger.warning(
                        f"Could not fit map bounds to sample points: {fit_bounds_err}"
                    )

            if hasattr(self, "stage3_output"):
                self.stage3_output.type = "success"
                self.stage3_output.add_msg(
                    f"Displayed {valid_points_added_count} sample points on the map."
                )
        else:
