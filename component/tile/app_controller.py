...
print(
    f"DEBUG AppController Top Level: _rasterio_available_map = {_rasterio_available_map}"
)
print(
    f"DEBUG AppController Top Level: _pyproj_available_proc = {_pyproj_available_proc}"
)
...
class AppController:
    def __init__(self, model: AppModel):
        ...
        if hasattr(self, "objective_widget"):
            ...
            print(
                "WARNING (AppController Init): objective_widget not yet created, cannot observe."
            )
    ...
    def _create_map_widget(self):
        ...
        try:
            ...
            self.map_widget = widgets.HTML(
                "<p style='color:red; font-weight:bold;'>Error initializing map widget.</p>"
            )
    ...
    def _create_stage3_widgets(self):
        ...
        self.update_map_btn = sui.Btn(
            "Show/Update Samples on Map",
            icon="mdi-map-marker-check",
            class_="ma-2",
            disabled=True,
        )
        self.show_map_overlay_btn = sui.Btn(
    ...
    def _create_stepper(self):
        ...
        self.step_1 = v.StepperStep(
            step=1, complete=False, children=[self.step_1_title]
        )
        self.step_2 = v.StepperStep(
            step=2, complete=False, children=[self.step_2_title]
        )
        self.step_3 = v.StepperStep(
            step=3, complete=False, children=[self.step_3_title]
        )

        try:
            step1_content_widgets = self._create_step_1_content()
        except Exception as e_step1:
            print(f"!!! FATAL ERROR creating Step 1 content: {e_step1}")
            traceback.print_exc()
            step1_content_widgets = [
                v.Html(tag="p", children=[f"Error creating UI Step 1: {e_step1}"])
            ]

        self.content_1 = v.StepperContent(step=1, children=step1_content_widgets)
        self.content_2 = v.StepperContent(
            step=2, children=[v.ProgressCircular(indeterminate=True, class_="ma-4")]
        )
        self.content_3 = v.StepperContent(
        ...
        self.stepper = v.Stepper(
            ...
            children=[
                ...
                v.StepperItems(
                    children=[self.content_1, self.content_2, self.content_3]
                ),
            ],
        )
    ...
    def _create_step_2_content(self):
        ...
        if self.model.raw_area_df is None or self.model.raw_area_df.empty:
            return [
                v.Html(tag="p", children=["Error: No area data loaded from Step 1."])
            ]
            ...
            v.ListItemGroup(
                children=[
                    v.Html(
                        class_="pl-4 pt-2",
                        tag="b",
                        children=["Expected User's Accuracy (for Neyman):"],
                    )
                ]
            )
        ]
        ...
        for _, row in area_df_sorted.iterrows():
            ...
            name_widget = v.TextField(
                label=f"Name for Code {map_code}",
                v_model=initial_name,
                outlined=True,
                dense=True,
                class_="ma-2",
            )
            ...
            def _make_observer(slider, text_w, code_val):
                def _observer(change):
                    new_name = (
                        text_w.v_model.strip() if text_w.v_model else f"Code {code_val}"
                    )
            ...
            name_widget.observe(
                _make_observer(acc_widget, name_widget, map_code), names="v_model"
            )

        edit_card = v.Card(
            outlined=True,
            class_="mb-4",
            children=[v.List(dense=True, children=name_items)],
        )
        ua_card = v.Card(
            outlined=True,
            class_="mb-4",
            children=[v.List(dense=True, children=ua_items)],
        )
        ...
        try:
            ...
            print(f"DEBUG: Error removing previous Stage 2 submit handler: {e}")
    ...
    def _create_step_3_content(self):
        ...
        if self.model.final_area_df is None or self.model.final_area_df.empty:
            return [
                v.Html(
                    tag="p",
                    children=[
                        "Error: Finalized class data is not available from Step 2."
                    ],
                )
            ]
        ...
        self.show_map_overlay_btn.disabled = not (
            self.model.file_path and self.base_map_overlay_layer
        )

        try:
            self.stage3_calculate_btn.on_event(
                "click", self._on_stage3_calculate, remove=True
            )
        except Exception as e:
            print(f"DEBUG: Error removing calc handler: {e}")
        self.stage3_calculate_btn.on_event("click", self._on_stage3_calculate)

        try:
            self.update_map_btn.on_event(
                "click", self._on_update_map_click, remove=True
            )
        except Exception as e:
            print(f"DEBUG: Error removing map update handler: {e}")
        self.update_map_btn.on_event("click", self._on_update_map_click)

        try:
            self.show_map_overlay_btn.on_event(
                "click", self._on_show_map_overlay_click, remove=True
            )
        except Exception as e:
            print(f"DEBUG: Error removing show map overlay handler: {e}")
    ...
    def _handle_objective_change(self, change):
        ...
        can_enable_target_class = (
            is_target_class_mode
            and len(getattr(self.target_class_widget, "items", [])) > 0
        ...
        print(
            f"DEBUG: Objective changed to '{change.get('new')}'. Target class widgets enabled: {not self.target_class_widget.disabled}"
        )
    ...
    def _update_target_class_options(self):
        ...
        if (
            ...
            try:
                ...
                target_widget.v_model = (
                    None
                    if not choices or choices[0].get("value") is None
    ...
    def _on_load_click(self, widget, event, data):
        ...
        if hasattr(self, "step_1"):
            self.step_1.complete = False
        if hasattr(self, "step_2"):
            self.step_2.complete = False
        if hasattr(self, "step_3"):
            self.step_3.complete = False

        layers_to_clear_on_load = []
        if self.sample_points_layer:
            layers_to_clear_on_load.append(self.sample_points_layer)
        if self.base_map_overlay_layer:
            layers_to_clear_on_load.append(self.base_map_overlay_layer)

        for lyr_to_clear in layers_to_clear_on_load:
            try:
                if lyr_to_clear in self.map_widget.layers:
                    self.map_widget.remove_layer(lyr_to_clear)
            except Exception as e:
                print(
                    f"Warning: Could not remove tracked layer '{getattr(lyr_to_clear, 'name', 'Unnamed')}': {e}"
                )
        self.sample_points_layer = None
        self.base_map_overlay_layer = None
        ...
        for lyr_general in current_map_layers_list:
            if (
                isinstance(lyr_general, (ImageOverlay, GeoJSON, LayerGroup))
                and getattr(lyr_general, "base", False) is False
            ):
                try:
                    self.map_widget.remove_layer(lyr_general)
                except Exception as e:
                    print(
                        f"Warning: Could not remove other layer '{getattr(lyr_general, 'name', 'Unnamed')}': {e}"
                    )
        ...
        try:
            ...
            assert (
                selected and Path(selected).exists()
            ...
            print(
                f"DEBUG Controller Flags Check BEFORE is_raster (in _on_load_click): _rasterio_available_map = {_rasterio_available_map}"
            )
            print(
                f"DEBUG Controller Flags Check BEFORE is_raster (in _on_load_click): _pyproj_available_proc = {_pyproj_available_proc}"
            )
            try:
                import rasterio
                ...
                print(
                    f"DEBUG Controller Import Check (in _on_load_click): FAILED to import rasterio.warp directly. Error: {e}"
                )
            # --- END DEBUG PRINTS ---

            area_data = compute_map_area(selected)
            assert (
                area_data is not None
                and isinstance(area_data, pd.DataFrame)
                and not area_data.empty
            ), "Area computation failed or returned empty."
            if "map_edited_class" not in area_data.columns:
                area_data["map_edited_class"] = area_data["map_code"].astype(str)
            ...
            if is_raster:
                ...
                print(
                    f"DEBUG _on_load_click (inside is_raster): _rasterio_available_map = {_rasterio_available_map}"
                )
                print(
                    f"DEBUG _on_load_click (inside is_raster): _pyproj_available_proc = {_pyproj_available_proc}"
                )
                    ...
                    try:
                        with rasterio.open(selected) as src:
                            if src.crs:
                                ...
                                overlay_bounds = [
                                    [dst_bounds[1], dst_bounds[0]],
                                    [dst_bounds[3], dst_bounds[2]],
                                ]
                            else:
                                print(
                                    "Warning: Raster missing CRS for bounds calculation."
                                )
                    except Exception as bounds_err:
                        print(f"Error calculating raster bounds: {bounds_err}")

                if overlay_bounds:
                    newly_added_overlay_layer = _add_overlay_layer(
                        self.map_widget,
                        selected,
                    ...
                    print(
                        "Skipping raster overlay due to missing bounds or dependencies."
                    )
                ...
                if _geopandas_available_proc:
                    try:
                        ...
                        if not gdf_loaded.empty:
                            if gdf_loaded.crs is None:
                                print("Warning: Vector has no CRS defined.")
                            vector_gdf = gdf_loaded.to_crs(
                                epsg=map_utils_target_crs_epsg
                            )
                    except Exception as gdf_err:
                        print(f"Error loading/reprojecting vector: {gdf_err}")
                ...
                if vector_gdf is not None:
                    ...
                    print(
                        "Skipping vector overlay due to loading/processing error, empty GDF, or missing GeoPandas."
                    )
            ...
            if newly_added_overlay_layer:
                ...
                print(
                    f"DEBUG: Loaded map overlay '{getattr(newly_added_overlay_layer, 'name', 'Unnamed')}' tracked."
                )
            else:
                self.show_map_overlay_btn.disabled = True
            ...
            self.model.alert_message = f"Error processing file: {e}"
            if hasattr(self, "step_1"):
                self.step_1.complete = False
        except Exception as e:
            ...
            traceback.print_exc()
            self.model.alert_type = "error"
            self.model.alert_message = f"Unexpected error: {e}"
            if hasattr(self, "step_1"):
                self.step_1.complete = False
        finally:
            self.alert_step1.type = self.model.alert_type
        ...
        try:
            if not hasattr(self, "class_name_widgets") or not hasattr(
                ...
                raise AttributeError(
                    "Stage 2 widgets (class names or UA sliders) not initialized properly."
                )

            edited_classes = {
                code: name_w.v_model.strip()
                if name_w.v_model and name_w.v_model.strip()
                else str(code)
            ...
            if raw_df_codes != slider_codes:
                print(
                    f"ERROR DEBUG: Slider keys {slider_codes} != Raw DF keys {raw_df_codes}"
                )
                raise ValueError("Mismatch between UA sliders and loaded map codes.")

            processed_df = self.model.raw_area_df.copy()
            processed_df["map_edited_class"] = processed_df["map_code"].map(
                edited_classes
            )
            assert (
                "map_edited_class" in processed_df.columns
                and not processed_df["map_edited_class"].isnull().any()
            ), "Mapping edited class names failed."
            ...
            self.stage2_alert.children = ["Class data submitted. Proceed to Stage 3."]
            if hasattr(self, "step_2"):
                self.step_2.complete = True
            if hasattr(self, "step_3"):
                self.step_3.complete = False
            self.model.current_step = 3
        except (AttributeError, ValueError, KeyError, AssertionError) as e:
            print(f"!!! Error processing Stage 2 data: {e}")
            traceback.print_exc()
            self.stage2_alert.type = "error"
            self.stage2_alert.children = [f"Error submitting class data: {e}"]
            if hasattr(self, "step_2"):
                self.step_2.complete = False
        except Exception as e:
            print(f"!!! An unexpected error occurred during Stage 2 submit: {e}")
            traceback.print_exc()
            self.stage2_alert.type = "error"
            self.stage2_alert.children = [f"Unexpected error: {e}"]
            if hasattr(self, "step_2"):
                self.step_2.complete = False
        finally:
            widget.loading = False
    ...
    def _on_stage3_calculate(self, widget, event, data):
        ...
        if self.sample_points_layer is not None:
            try:
                ...
                print(f"Warning: Could not remove old sample points layer: {e}")
        ...
        try:
            ...
            assert (
                self.model.final_area_df is not None
            ), "Model is missing the final area data."
            ...
            if proc.sample_points_df is not None and isinstance(
                proc.sample_points_df, pd.DataFrame
            ):
                self.model.sample_points_df = proc.sample_points_df.copy()
                if not self.model.sample_points_df.empty:
                    if hasattr(self, "step_3"):
                        self.step_3.complete = True
            ...
            print(f"!!! Error during Stage 3 Calculate: {e}")
            traceback.print_exc()
            self.stage3_output.type = "error"
            self.stage3_output.children = [f"Error during calculation: {e}"]
            if hasattr(self, "step_3"):
                self.step_3.complete = False
            self.update_map_btn.disabled = True
        finally:
    ...
    def _on_show_map_overlay_click(self, widget, event, data):
        ...
        try:
            if self.base_map_overlay_layer is not None:
                try:
                    if self.base_map_overlay_layer in self.map_widget.layers:
                        ...
                        print(
                            "DEBUG: Removed existing base map overlay layer for refresh."
                        )
                    self.base_map_overlay_layer = None
                except Exception as e:
                    print(
                        f"Warning: Could not remove existing base map overlay layer for refresh: {e}"
                    )
            ...
            if is_raster:
                ...
                if not (_rasterio_available_map and _pyproj_available_proc):
                    ...
                    self.stage3_output.add_msg(
                        "Rasterio/Pyproj missing for raster overlay refresh."
                    )
                    widget.loading = False
                    return
                ...
                try:
                    with rasterio.open(self.model.file_path) as src_refresh:
                        if src_refresh.crs:
                            dst_bounds_refresh = rasterio.warp.transform_bounds(
                                src_refresh.crs,
                                f"EPSG:{map_utils_target_crs_epsg}",
                                *src_refresh.bounds,
                            )
                            overlay_bounds_refresh = [
                                [dst_bounds_refresh[1], dst_bounds_refresh[0]],
                                [dst_bounds_refresh[3], dst_bounds_refresh[2]],
                            ]
                        else:
                            self.stage3_output.add_msg(
                                "Warning: Raster missing CRS, cannot calculate accurate bounds for refresh.",
                                "warning",
                            )
                except Exception as bounds_err_refresh:
                    self.stage3_output.add_msg(
                        f"Error calculating raster bounds for refresh: {bounds_err_refresh}",
                        "error",
                    )
                    print(f"Error calc bounds for refresh: {bounds_err_refresh}")

                if overlay_bounds_refresh and self.model.raw_area_df is not None:
                    new_refreshed_overlay_layer = _add_overlay_layer(
                        self.map_widget,
                    ...
                    self.stage3_output.add_msg(
                        "Could not generate bounds or missing legend data for raster overlay refresh.",
                        "warning",
                    )
                ...
                if not _geopandas_available_proc:
                    self.stage3_output.type = "error"
                    self.stage3_output.add_msg(
                        "GeoPandas missing for vector overlay refresh."
                    )
                    widget.loading = False
                    return
                ...
                try:
                    ...
                    if not gdf_loaded_refresh.empty:
                        if gdf_loaded_refresh.crs is None:
                            self.stage3_output.add_msg(
                                "Warning: Vector has no CRS defined for refresh.",
                                "warning",
                            )
                        vector_gdf_refresh = gdf_loaded_refresh.to_crs(
                            epsg=map_utils_target_crs_epsg
                        )
                except Exception as gdf_err_refresh:
                    self.stage3_output.add_msg(
                        f"Error loading/reprojecting vector for refresh: {gdf_err_refresh}",
                        "error",
                    )
                    print(f"Error loading vector for refresh: {gdf_err_refresh}")

                if (
                    vector_gdf_refresh is not None
                    and self.model.raw_area_df is not None
                ):
                    new_refreshed_overlay_layer = _add_overlay_layer(
                        self.map_widget,
                        self.model.file_path,
                    ...
                    self.stage3_output.add_msg(
                        "Could not load/process vector or missing legend data for overlay refresh.",
                        "warning",
                    )
                ...
                self.stage3_output.add_msg(
                    f"File type of '{basename}' not recognized for overlay refresh."
                )
            ...
            if new_refreshed_overlay_layer:
                ...
                self.stage3_output.add_msg(
                    f"Map overlay '{basename}' refreshed successfully."
                )
            else:
                if self.stage3_output.type != "error":
                    self.stage3_output.type = "warning"
                self.stage3_output.add_msg(
                    f"Failed to refresh map overlay for '{basename}'. Check logs."
                )

        except Exception as e:
            print(f"!!! Error refreshing map overlay: {e}")
            traceback.print_exc()
            self.stage3_output.type = "error"
            self.stage3_output.add_msg(f"Error refreshing overlay: {e}")
        finally:
            widget.loading = False
    ...
    def _on_update_map_click(self, widget, event, data):
        ...
        try:
            ...
            print(f"!!! Error updating map display with samples: {e}")
            traceback.print_exc()
            self.stage3_output.type = "error"
            self.stage3_output.add_msg(f"Error updating sample display: {e}")
        finally:
            widget.loading = False
    ...
    def _update_map_display_ipyleaflet(self):
        ...
        if self.sample_points_layer is not None:
            try:
                ...
                print(f"Warning: Could not remove previous sample points layer: {e}")

        points_df_local = self.model.sample_points_df
        if points_df_local is None or points_df_local.empty:
            self.stage3_output.type = "warning"
            self.stage3_output.add_msg("No sample points available to display.")
            return
        ...
        if not all(col in points_df_local.columns for col in required_cols):
            missing_cols_samples = [
                col for col in required_cols if col not in points_df_local.columns
            ]
            self.stage3_output.type = "error"
            self.stage3_output.add_msg(
                f"Sample points DataFrame is missing required columns: {missing_cols_samples}."
            )
            return
        ...
        try:
            ...
            print(f"Error generating class colors for map samples: {color_err_samples}")
            class_colors_samples = {}
            default_color_samples = "#FF0000"

        markers_list = []
        min_lat_samples, max_lat_samples = 90.0, -90.0
        min_lon_samples, max_lon_samples = 180.0, -180.0
        points_added_count = 0
        for _, row_sample in points_df_local.iterrows():
            try:
                lat_sample = float(row_sample["latitude"])
                lon_sample = float(row_sample["longitude"])
                if not (
                    np.isfinite(lat_sample)
                    and np.isfinite(lon_sample)
                    and -90 <= lat_sample <= 90
                    and -180 <= lon_sample <= 180
                ):
                    print(
                        f"Warning: Skipping sample point with invalid coords: lat={lat_sample}, lon={lon_sample}"
                    )
                    continue

                min_lat_samples = min(min_lat_samples, lat_sample)
                max_lat_samples = max(max_lat_samples, lat_sample)
                min_lon_samples = min(min_lon_samples, lon_sample)
                max_lon_samples = max(max_lon_samples, lon_sample)

                map_class_sample = row_sample["map_edited_class"]
                map_code_sample = row_sample["map_code"]
                color_sample = class_colors_samples.get(
                    map_class_sample, default_color_samples
                )

                popup_content_html_sample = (
                    f"<b>Class:</b> {html.escape(str(map_class_sample))}<br>"
                    f"<b>Code:</b> {html.escape(str(map_code_sample))}<br>"
                    f"<b>Lat:</b> {lat_sample:.6f}<br><b>Lon:</b> {lon_sample:.6f}"
                )
                marker_sample = CircleMarker(
                    location=(lat_sample, lon_sample),
                    radius=5,
                    color=color_sample,
                    weight=1,
                    fill_color=color_sample,
                    fill_opacity=0.8,
                    popup=widgets.HTML(value=popup_content_html_sample),
                )
        ...
        if markers_list:
            self.sample_points_layer = LayerGroup(
                layers=markers_list, name="Sample Points"
            )
            self.map_widget.add_layer(self.sample_points_layer)
            if (
                min_lat_samples <= max_lat_samples
                and min_lon_samples <= max_lon_samples
            ):
                bounds_for_samples = [
                    [min_lat_samples, min_lon_samples],
                    [max_lat_samples, max_lon_samples],
                ]
                if points_added_count == 1:
                    padding_for_samples = 0.01
                    bounds_for_samples = [
                        [
                            min_lat_samples - padding_for_samples,
                            min_lon_samples - padding_for_samples,
                        ],
                        [
                            max_lat_samples + padding_for_samples,
                            max_lon_samples + padding_for_samples,
                        ],
                    ]
                try:
                    ...
                    print(
                        f"Warning: Could not fit map bounds to samples: {bounds_err_samples}"
                    )
            self.stage3_output.type = "success"
            self.stage3_output.add_msg(
                f"Displayed {points_added_count} sample points on the map."
            )
        else:
            self.stage3_output.type = "warning"
            self.stage3_output.add_msg(
                "No valid sample points found to display on the map."
            )
